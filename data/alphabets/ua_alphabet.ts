import { ExampleType } from './cz_alphabet';

interface AplhabetType {
  letter: [string, string | null];
  transcription: string;
  examples: ExampleType[];
}

export const ALPHABET_UA: AplhabetType[] = [
  {
    letter: ['A', 'a'],
    transcription: '[a]',
    examples: [
      {
        example: 'алфавіт',
        example_transcription: 'alfavit',
      },
      {
        example: 'автомобіль',
        example_transcription: 'avtomobiľ',
      },
      {
        example: 'ананас',
        example_transcription: 'ananas',
      },
    ],
  },
  {
    letter: ['Б', 'б'],
    transcription: '[b]',
    examples: [
      {
        example: 'банан',
        example_transcription: 'banan',
      },
      {
        example: 'будинок',
        example_transcription: 'budynok',
      },
      {
        example: 'безліч',
        example_transcription: 'bezlič',
      },
    ],
  },
  {
    letter: ['В', 'в'],
    transcription: '[v]',
    examples: [
      {
        example: 'вода',
        example_transcription: 'voda',
      },
      {
        example: 'виделка',
        example_transcription: 'vydelka',
      },
      {
        example: 'вулиця',
        example_transcription: 'vulytsja',
      },
    ],
  },
  {
    letter: ['Г', 'г'],
    transcription: '[h]',
    examples: [
      {
        example: 'голова',
        example_transcription: 'holova',
      },
      {
        example: 'грати',
        example_transcription: 'hraty',
      },
      {
        example: 'готель',
        example_transcription: 'hoteľ',
      },
    ],
  },
  {
    letter: ['Ґ', 'ґ'],
    transcription: '[g]',
    examples: [
      {
        example: 'ґумка',
        example_transcription: 'gumka',
      },
      {
        example: 'ґанок',
        example_transcription: 'ganok',
      },
      {
        example: 'ґудзик',
        example_transcription: 'gudzyk',
      },
    ],
  },
  {
    letter: ['Д', 'д'],
    transcription: '[d]',
    examples: [
      {
        example: 'дім',
        example_transcription: 'dim',
      },
      {
        example: 'дерево',
        example_transcription: 'derevo',
      },
      {
        example: 'добрий',
        example_transcription: 'dobryj',
      },
    ],
  },
  {
    letter: ['Е', 'е'],
    transcription: '[е]',
    examples: [
      {
        example: 'екскаватор',
        example_transcription: 'ekskavator',
      },
      {
        example: 'екватор',
        example_transcription: 'ekvator',
      },
      {
        example: 'дерево',
        example_transcription: 'derevo',
      },
    ],
  },
  {
    letter: ['Є', 'є'],
    transcription: '[je]',
    examples: [
      {
        example: 'Європа',
        example_transcription: 'jevropa',
      },
      {
        example: 'єдиний',
        example_transcription: 'jedynyj',
      },
      {
        example: 'життєвий',
        example_transcription: 'žytěvyj',
      },
    ],
  },
  {
    letter: ['Ж', 'ж'],
    transcription: '[ž]',
    examples: [
      {
        example: 'жінка',
        example_transcription: 'žinka',
      },
      {
        example: 'життя',
        example_transcription: 'žyťa',
      },
      {
        example: 'між',
        example_transcription: 'miž',
      },
    ],
  },
  {
    letter: ['З', 'з'],
    transcription: '[z]',
    examples: [
      {
        example: 'зуб',
        example_transcription: 'zub',
      },
      {
        example: 'залізниця',
        example_transcription: 'zaliznytsja',
      },
      {
        example: 'вокзал',
        example_transcription: 'vokzal',
      },
    ],
  },
  {
    letter: ['И', 'и'],
    transcription: '[y]',
    examples: [
      {
        example: 'сир',
        example_transcription: 'syr',
      },
      {
        example: 'невинний',
        example_transcription: 'nevynnyj',
      },
      {
        example: 'кмітливий',
        example_transcription: 'kmitlyvyj',
      },
    ],
  },
  {
    letter: ['І', 'і'],
    transcription: '[i]',
    examples: [
      {
        example: 'Інтернет',
        example_transcription: 'internet',
      },
      {
        example: 'ігра',
        example_transcription: 'ihra',
      },
      {
        example: 'цікавий',
        example_transcription: 'cikavyj',
      },
    ],
  },
  {
    letter: ['Ї', 'ї'],
    transcription: '[ji]',
    examples: [
      {
        example: 'їжак',
        example_transcription: 'jižak',
      },
      {
        example: 'їсти',
        example_transcription: 'jisty',
      },
      {
        example: 'їхати',
        example_transcription: 'jichaty',
      },
    ],
  },
  {
    letter: ['Й', 'й'],
    transcription: '[j]',
    examples: [
      {
        example: 'йогурт',
        example_transcription: 'johurt',
      },
      {
        example: 'інтересний',
        example_transcription: 'interesnyj',
      },
      {
        example: 'довгий',
        example_transcription: 'dovhyj',
      },
    ],
  },
  {
    letter: ['К', 'к'],
    transcription: '[k]',
    examples: [
      {
        example: 'кімната',
        example_transcription: 'kimnata',
      },
      {
        example: 'ковдра',
        example_transcription: 'kovdra',
      },
      {
        example: 'камінь',
        example_transcription: 'kamiň',
      },
    ],
  },
  {
    letter: ['Л', 'л'],
    transcription: '[l]',
    examples: [
      {
        example: 'любов',
        example_transcription: 'ľubov',
      },
      {
        example: 'літо',
        example_transcription: 'lito',
      },
      {
        example: 'лопата',
        example_transcription: 'lopata',
      },
    ],
  },
  {
    letter: ['М', 'м'],
    transcription: '[m]',
    examples: [
      {
        example: 'мова',
        example_transcription: 'mova',
      },
      {
        example: 'місто',
        example_transcription: 'misto',
      },
      {
        example: 'музика',
        example_transcription: 'muzyka',
      },
    ],
  },
  {
    letter: ['Н', 'н'],
    transcription: '[n]',
    examples: [
      {
        example: 'новини',
        example_transcription: 'novyny',
      },
      {
        example: 'ніс',
        example_transcription: 'nis',
      },
      {
        example: 'ножик',
        example_transcription: 'nožyk',
      },
    ],
  },
  {
    letter: ['О', 'о'],
    transcription: '[о]',
    examples: [
      {
        example: 'озеро',
        example_transcription: 'ozero',
      },
      {
        example: 'овочі',
        example_transcription: 'ovoči',
      },
      {
        example: 'повінь',
        example_transcription: 'poviň',
      },
    ],
  },
  {
    letter: ['П', 'п'],
    transcription: '[p]',
    examples: [
      {
        example: 'правда',
        example_transcription: 'pravda',
      },
      {
        example: 'паста',
        example_transcription: 'pasta',
      },
      {
        example: 'помідор',
        example_transcription: 'pomidor',
      },
    ],
  },
  {
    letter: ['Р', 'р'],
    transcription: '[r]',
    examples: [
      {
        example: 'риба',
        example_transcription: 'ryba',
      },
      {
        example: 'робота',
        example_transcription: 'robota',
      },
      {
        example: 'ринок',
        example_transcription: 'rynok',
      },
    ],
  },
  {
    letter: ['С', 'с'],
    transcription: '[s]',
    examples: [
      {
        example: 'село',
        example_transcription: 'selo',
      },
      {
        example: 'сонце',
        example_transcription: 'sonce',
      },
      {
        example: 'сіль',
        example_transcription: 'siľ',
      },
    ],
  },
  {
    letter: ['T', 'т'],
    transcription: '[t]',
    examples: [
      {
        example: 'тато',
        example_transcription: 'tato',
      },
      {
        example: 'туман',
        example_transcription: 'tuman',
      },
      {
        example: 'телевізор',
        example_transcription: 'televizor',
      },
    ],
  },
  {
    letter: ['У', 'у'],
    transcription: '[u]',
    examples: [
      {
        example: 'Україна',
        example_transcription: 'ukrajina',
      },
      {
        example: 'удав',
        example_transcription: 'udav',
      },
      {
        example: 'дуб',
        example_transcription: 'dub',
      },
    ],
  },
  {
    letter: ['Ф', 'ф'],
    transcription: '[f]',
    examples: [
      {
        example: 'фізика',
        example_transcription: 'fizyka',
      },
      {
        example: 'фаворит',
        example_transcription: 'favoryt',
      },
      {
        example: 'Фінляндія',
        example_transcription: 'finľandija',
      },
    ],
  },
  {
    letter: ['Х', 'х'],
    transcription: '[ch]',
    examples: [
      {
        example: 'хліб',
        example_transcription: 'chlib',
      },
      {
        example: 'ховати',
        example_transcription: 'chovaty',
      },
      {
        example: 'хвіст',
        example_transcription: 'chvist',
      },
    ],
  },
  {
    letter: ['Ц', 'ц'],
    transcription: '[c]',
    examples: [
      {
        example: 'цибуля',
        example_transcription: 'cybuľa',
      },
      {
        example: 'церква',
        example_transcription: 'cerkva',
      },
      {
        example: 'цукор',
        example_transcription: 'cukor',
      },
    ],
  },
  {
    letter: ['Ч', 'ч'],
    transcription: '[č]',
    examples: [
      {
        example: 'Чехія',
        example_transcription: 'čechija',
      },
      {
        example: 'чесний',
        example_transcription: 'česnyj',
      },
      {
        example: 'читати',
        example_transcription: 'čytaty',
      },
    ],
  },
  {
    letter: ['Ш', 'ш'],
    transcription: '[š]',
    examples: [
      {
        example: 'шампунь',
        example_transcription: 'šampuň',
      },
      {
        example: 'шина',
        example_transcription: 'šyna',
      },
      {
        example: 'кошик',
        example_transcription: 'košyk',
      },
    ],
  },
  {
    letter: ['Щ', 'щ'],
    transcription: '[šč]',
    examples: [
      {
        example: 'щука',
        example_transcription: 'ščuka',
      },
      {
        example: 'кущ',
        example_transcription: 'kušč',
      },
      {
        example: 'щенок',
        example_transcription: 'ščenok',
      },
    ],
  },
  {
    letter: ['ь', null],
    transcription: 'měkký znak',
    examples: [
      {
        example: 'сіль',
        example_transcription: 'siľ',
      },
      {
        example: 'лінь',
        example_transcription: 'ľiň',
      },
      {
        example: 'вільний',
        example_transcription: 'viľnyj',
      },
    ],
  },
  {
    letter: ['Ю', 'ю'],
    transcription: '[ju]',
    examples: [
      {
        example: 'юнак',
        example_transcription: 'junak',
      },
      {
        example: 'ювілей',
        example_transcription: 'juvilej',
      },
      {
        example: 'любити',
        example_transcription: 'ľubyty',
      },
    ],
  },
  {
    letter: ['Я', 'я'],
    transcription: '[ja]',
    examples: [
      {
        example: 'юнак',
        example_transcription: 'junak',
      },
      {
        example: 'ювілей',
        example_transcription: 'juvilej',
      },
      {
        example: 'любити',
        example_transcription: 'ľubyty',
      },
    ],
  },
];
